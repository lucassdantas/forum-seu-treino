...
export const UserInFollowContextCard = ({user, className}:{user:UserInFollowContextProps, className?:string}) => {
  return (
    <div className={'bg-neutral-950 text-white rounded-md' + className}>
      <div className={`bg-[url("${user.coverPhoto} bg-neutral-400 w-full h-12 p-1")]`}> </div>
      <div className='p-4 flex flex-col items-center gap-4 w-full'>
        <img src={user.userImage } alt={'Foto de perfil do' + user.userName} className='w-[120px] -mt-12 text-center relative'/>
