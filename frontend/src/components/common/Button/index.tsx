...
export const Button = ({children, className, onClick}:ButtonProps) => {
  return (
    <div onClick={() => onClick?.()} className={`bg-orange-seu-treino rounded-lg text-center text-black px-2 py-2 max-w-[160px] hover:bg-yellow-seu-treino transition-colors font-semibold cursor-pointer text-lg ${className}`}>
       <span>{children}</span>
    </div>
  )
}
