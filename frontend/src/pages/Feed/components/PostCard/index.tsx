    ...
    <div>
      <GrayCard className=''>
        ...
        <div className='flex border-t border-t-neutral-700 py-4 gap-4'>
          ...
          <div className='flex items-center gap-2'>
            <TfiComment className='cursor-pointer'/> <span>{post.commentsQuantity} Comentário{post.commentsQuantity===1? 's':''} </span>
          </div>
        </div>
      </GrayCard>
      {commentsWithAuthors.length>0 &&<PostAnswer commentWithAuthor={commentWithAuthor} commentsQuantity={post.commentsQuantity} currentPost={post} commentsWithAuthorsOnComponent={commentsWithAuthorsOnComponent} setCommentsWithAuthorsOnComponent={setCommentsWithAuthorsOnComponent}/>}
    </div>
  )
}

const PostAnswer = ({commentWithAuthor, commentsQuantity, currentPost, commentsWithAuthorsOnComponent, setCommentsWithAuthorsOnComponent}:{commentWithAuthor:CommentsWithAuthors, commentsQuantity:number, currentPost:PostWithAuthors, commentsWithAuthorsOnComponent:CommentsWithAuthors[], setCommentsWithAuthorsOnComponent:any}) => {
